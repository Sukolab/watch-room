    ...
    (function(){
      ...
      var els={nameInput:document.getElementById('nameInput'),roomInput:document.getElementById('roomInput'),passwordInput:document.getElementById('passwordInput'),deviceMode:document.getElementById('deviceMode'),generateRoomBtn:document.getElementById('generateRoomBtn'),copyRoomBtn:document.getElementById('copyRoomBtn'),joinBtn:document.getElementById('joinBtn'),shareBtn:document.getElementById('shareBtn'),micBtn:document.getElementById('micBtn'),camBtn:document.getElementById('camBtn'),leaveBtn:document.getElementById('leaveBtn'),remoteVideo:document.getElementById('remoteVideo'),camVideo:document.getElementById('camVideo'),status:document.getElementById('status'),remoteState:document.getElementById('remoteState'),camState:document.getElementById('camState'),countValue:document.getElementById('countValue'),roleValue:document.getElementById('roleValue'),roomsContainer:document.getElementById('roomsContainer'),refreshRoomsBtn:document.getElementById('refreshRoomsBtn'),screenTitle:document.getElementById('screenTitle'),camTitle:document.getElementById('camTitle')};
      ...
      function safePlay(video){ if(!video) return Promise.resolve(); try{ var p=video.play(); if(p && typeof p.catch==='function'){ return p.catch(function(){}) } }catch(e){} return Promise.resolve(); }
      ...
      function clearRemoteView(){
        remoteScreenStream=new MediaStream(); remoteCamStream=new MediaStream();
      ...
      function createPeer(kind){
        ...
        pc.ontrack=function(event){
          ...
          if(kind==='main'){
            if(stream){ stream.getTracks().forEach(function(track){ if(!remoteScreenStream.getTracks().some(function(t){return t.id===track.id;})) remoteScreenStream.addTrack(track); }); }
      ...
      function syncLocalTracks(){
        ensurePeers();
        try{ mainSenders.audio && mainSenders.audio.replaceTrack(localAudioStream && localAudioStream.getAudioTracks()[0] || null); }catch(e){}
      ...
      async function getMicStream(){ return await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true},video:false}); }
      ...
      async function startScreenShare(){
        try{
          ...
          screenStream=await navigator.mediaDevices.getDisplayMedia({video:{frameRate:{ideal:20,max:24}},audio:true});
          var v=screenStream.getVideoTracks()[0]; if(v){ try{ v.contentHint='detail'; }catch(e){} v.onended=function(){ stopScreenShare(); }; }
          isSharing=true; syncLocalTracks(); attachScreenStream(screenStream,'Preview'); emitMediaState(); updateUiState(); forceNegotiate('main'); setStatus('Screen sharing started.');
        }catch(err){ setStatus('Screen share failed: '+((err&&err.message)?err.message:String(err))); }
      }
      async function stopScreenShare(){ stopTracks(screenStream); screenStream=null; isSharing=false; syncLocalTracks(); attachScreenStream(isHost?new MediaStream():new MediaStream(),'Waiting'); emitMediaState(); updateUiState(); forceNegotiate('main'); setStatus('Screen sharing stopped.'); }
      async function toggleScreenShare(){ if(!inRoom) return; if(isSharing) await stopScreenShare(); else await startScreenShare(); }
      async function toggleMic(){
        if(!inRoom) return;
        if(isMicOn){ stopTracks(localAudioStream); localAudioStream=null; isMicOn=false; syncLocalTracks(); forceNegotiate('main'); updateUiState(); setStatus('Microphone disabled.'); return; }
        try{ localAudioStream=await getMicStream(); isMicOn=true; syncLocalTracks(); forceNegotiate('main'); updateUiState(); setStatus('Microphone enabled.'); }
      ...
      function resetAll(){
        ...
        localAudioStream=null; screenStream=null; camStream=null; inRoom=false; joinedRole='viewer'; isHost=false; isMicOn=false; isSharing=false; isCamOn=false; setRole('Viewer'); setCount(0); updateUiState(); clearRemoteView();
      }
